                ...
                if (BusinessObjectInfo.ActionSuccess == true)
                {
                    if (!string.IsNullOrEmpty(SBOMain.TFromUID) && SBOMain.FromCnt != null)
                    {
                        ...
                        if (SBOMain.sForm == "LC")
                        {
                            ...
                            string docnum = objCU.getDocNumFromDocKey("OPQT", DocEntry);
                            string currency = objCU.getCurrFromDocKey("OPQT", DocEntry);
                            double lineTotal = objCU.getLineTotalFromDocKey("PQT1", DocEntry); 
                            double rate = objCU.getRateFromDocKey("PQT1", DocEntry);
