...
namespace CoreSuteConnect.Class.EXIM
{
    class clsPortMaster
    {
        ...
        public clsPortMaster(OutwardToPortMaster inClass)
        {
            if (inClass != null)
            {
                oForm = SBOMain.SBO_Application.Forms.ActiveForm;
                oForm.Mode = SAPbouiCOM.BoFormMode.fm_FIND_MODE;
                oForm.Items.Item("tportcode").Specific.value = inClass.portcode;
                       ...
                       if (pVal.BeforeAction == true)
                        {
                            ...
                            {
                                if (string.IsNullOrEmpty(oForm.Items.Item("tportcode").Specific.value))
                                {
                                    BubbleEvent = false;
                                    SBOMain.SBO_Application.StatusBar.SetText("Please Add Port Code", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
                                    oForm.Items.Item("tportcode").Click();
                                }
                                else if (string.IsNullOrEmpty(oForm.Items.Item("tportname").Specific.value))
                                {
                                    BubbleEvent = false;
                                    SBOMain.SBO_Application.StatusBar.SetText("Please Add Port Name", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
                                    oForm.Items.Item("tportname").Click();
                                } 
                            }
                        }
                        break; 
